...
function setDefaultYearToCurrentYear() {
  ...
  const minYear = Number.parseInt(yearInput.min || '1583', 10);
  const maxYear = Number.parseInt(yearInput.max || '4099', 10);
...
function clampYearValue(year) {
  const yearInput = document.getElementById('year');
  const minYear = Number.parseInt(yearInput.getAttribute('min') || '1583', 10);
  const maxYear = Number.parseInt(yearInput.getAttribute('max') || '4099', 10);
...
function stepYear(delta) {
  ...
  const baseYear = Number.isInteger(parsed) ? parsed : new Date().getFullYear();
  yearInput.value = String(clampYearValue(baseYear + delta));
  calculateAndDisplay({ silentInvalid: true });
}

function wireYearStepper() {
  const yearInput = document.getElementById('year');
  const buttons = document.querySelectorAll('[data-year-step]');

  buttons.forEach(button => {
    let repeatDelay = null;
    let repeatInterval = null;
    const delta = Number.parseInt(button.dataset.yearStep || '0', 10);
    ...
    const startRepeating = event => {
      ...
      repeatDelay = window.setTimeout(() => {
        repeatInterval = window.setInterval(() => stepYear(delta), 120);
      }, 450);
    };
...
function wireUpUi() {
  ...
  yearInput.addEventListener('input', autoUpdateCalendar);
