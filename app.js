...
function calculateFeasts(year, easter) {
  ...
  return [
    ...
    { feast: 'St. James of Nisibis', date: saturdayOnOrAfter(makeDate(year, 12, 12)) }
  ];
}
...
function wireUpUi() {
  ...
  document.getElementById('year').addEventListener('keydown', event => {
    if (event.key === 'Enter') {
      calculateAndDisplay();
    }
  });
